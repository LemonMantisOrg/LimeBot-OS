...
**Chat photo delivery:** `web_search(kind="images")` with at least one image URL is the delivery signal. The host downloads the best hit through the SSRF-guarded fetch and delivers it even when a verb matcher would miss the prompt, the model writes no wrap-up, and it never calls `send_media`. Web stamps `metadata.image` + `attachments`. Discord/WhatsApp send the file. Exclusive-tool shortlist uses a photo noun (foto/photo/pic/picture/image/imagen) and not generate/draw; verb lists are not the classifier. That is not `generate_image`, `spawn_agent`, `send_media`, or `run_command`. Live chat media fetch is not a filesystem write and does not need confirmation. Web photo-send turns do not register `send_media`. Discord/WhatsApp may still expose `send_media` as a leftover file-share tool. Public-figure photo search stays allowed.
