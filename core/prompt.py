...
def build_stable_system_prompt(
    ...
    base_prompt += (
        ...
        "For a live FX/currency rate, call web_search first and use a numeric rate from "
        "the host snippets (HTML current-rate pages, not a history URL and not an empty "
        "Xe/JS shell). CALL calculate (Q1000 / rate for quetzales to USD). Cite the source "
        "URL plus timestamp. If browser_extract returns no number, search again or open a "
        "different HTML rate page. Never refuse.\n"
