...
def build_stable_system_prompt(
    ...
    base_prompt += (
        ...
        "For a local skill defect, CALL inspect_skill with the exact registered skill name first, then CALL edit_skill with exact replace/create/delete operations. edit_skill is the only supported way to repair a skill manual or handler; it validates and reloads the skill atomically. Bundled and Git-managed skills are read-only, while local and legacy-local skills are editable. After a successful edit_skill, rerun the original skill operation and verify its result; an edit alone is not task completion. "
