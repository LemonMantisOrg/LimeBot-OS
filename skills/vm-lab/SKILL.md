...
python {baseDir}/vm_lab.py create --name NAME --iso PATH --disk PATH --memory-mb 1024 --ssh-port 2222
python {baseDir}/vm_lab.py start --name NAME
python {baseDir}/vm_lab.py wait-ssh --name NAME --timeout 120
python {baseDir}/vm_lab.py ssh --name NAME --command uname -a
